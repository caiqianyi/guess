...
public class GuessOrderServiceImpl implements IGuessOrderService {
	...
	public Pager findByUserIdForPager(String userId, Integer status,
		...
		List<GuessOrder> datas = guessOrderMapper.findByForPager(orderParam, start, end, pager);
	...
	public boolean insert(GuessOrder order) {
		...
		option.setId(order.getOptionId()); 
		...
		gto.setBuyAmount(guessTopicOption.getBuyAmount().add(new BigDecimal(order.getDiamond())));
